...
const rgx = {
    ...
    sentences: /[^\.\!\?]*[\.\!\?]/gi,
    words: /[\w-]+/gi,
    qualifiers: new RegExp(Object.keys(getQualifyingWords()).join('|'), 'gim'),
    complexWords: new RegExp(Object.keys(getComplexWords()).join('|'), 'gim'),
    passiveVoice: new RegExp(`(${passivePreWords.join('|')})\\s+([\\w-]+ed)`, 'gim')
}
...
function getComplex(txt, offset = 0) {
    const wordAlts = getComplexWords()
...
function getAdverbs(txt, offset = 0) {
    ...
    const qualifierPhrases = getQualifyingWords();

    const adverbs = getWords(txt, offset)
        .filter(b => {
            const lowerTxt = b.txt.toLowerCase()
            return lowerTxt.endsWith('ly') && !lyWords[lowerTxt]
        })
    return adverbs
}
